...
fn test_batch_publish() {
    ...
    let create_price_feed = |seeds, index| {
    ...
    let price_feeds = [
        create_price_feed(b"seeds_1", 1),
        create_price_feed(b"seeds_2", 2),
        create_price_feed(b"seeds_3", 3),
        create_price_feed(b"seeds_4", 0),
    ];

    bank = new_from_parent(&Arc::new(bank)); // Advance slot 1.
    bank = new_from_parent(&Arc::new(bank)); // Advance slot 2.

    let new_price_feed1_account = bank.get_account(&price_feeds[0]).unwrap();
    ...
    let new_price_feed2_account = bank.get_account(&price_feeds[1]).unwrap();
    ...
    let new_price_feed4_account = bank.get_account(&price_feeds[3]).unwrap();
