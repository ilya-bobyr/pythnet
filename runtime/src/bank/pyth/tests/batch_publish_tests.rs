...
fn test_batch_publish() {
    ...
    let generate_publisher = |seed, seed2, new_prices| {
        ...
        publisher_config::create(
            publisher_config_account.data_as_mut_slice(),
            publisher_key.pubkey().to_bytes(),
            ...
            AccountSharedData::new(42, buffer::size(100), &PRICE_STORE_PID);
        {
            let (header, prices) = buffer::create(
                publisher_buffer_account.data_as_mut_slice(),
                publisher_key.pubkey().to_bytes(),
            )
            .unwrap();
    ...
    let new_price_feed1_account = bank.get_account(&prices_with_messages[0]).unwrap();
    ...
    let new_price_feed2_account = bank.get_account(&prices_with_messages[1]).unwrap();
    ...
    let new_price_feed4_account = bank.get_account(&prices_with_messages[3]).unwrap();
